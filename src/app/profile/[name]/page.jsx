...
function page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axios.get(`https://gymstore-production.up.railway.app/api/get_user_profile/${name}`);
        setProfile(data.profile);
        setGigs(data.gigs);

        const reviewResponse = await axios.get(
          `https://gymstore-production.up.railway.app/api/get_profile_review/${data.profile.id}`
        );
    ...
    <div className="flex flex-col md:flex-row justify-between py-5 px-8 gap-6 bg-gray-100 min-h-screen">
      <div className="flex flex-col gap-6 bg-white p-6 rounded-lg shadow-md w-full md:w-1/3 max-w-[320px]">
        <div className="flex flex-col items-center">
          <Avata width={100} height={100} image_url={profile?.avatar || '/guest_avatar.png'} />
